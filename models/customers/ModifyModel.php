...
<?php
	...
	class modifyModel {
		...
		*
		*/
		public function modify_model($cust_name, $cust_address, $cust_postal_code, $cust_city, $cust_phone_number, $cust_record_number) {
			try {
				$model = $this->db->prepare('UPDATE `camping`.`customer` SET cust_name = ?,cust_address = ?,cust_postal_code = ?,cust_city =?, cust_phone_number = ?,cust_record_number = ?  WHERE `cust_name` = ?');
				...
				$model->bindValue(7, $cust_name, PDO::PARAM_STR);
				$model->execute();
				$model->closeCursor();
			}
